...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x, layer, tsn_mode=False, finetune=False):
        if tsn_mode:
        ...
        if tsn_mode:
            ...
            x_tsn1 = nn.functional.normalize(self.fc_tsn(x_tsn), p=2, dim=1)
            ...
            if self.order_out:
                ...
                out = [x_tsn1]
                if self.intra_out:
                     out.append(nn.functional.normalize(self.fc_intra(x_tsn), p=2, dim=1))
                # out = x_tsn
        else:
    ...
    def forward(self, x, layer=7, order_feat=None, tsn_mode=False,finetune=False):
        out = self._forward_impl(x, layer, tsn_mode, finetune)
